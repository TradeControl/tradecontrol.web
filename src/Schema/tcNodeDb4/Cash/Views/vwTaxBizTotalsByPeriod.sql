CREATE VIEW Cash.vwTaxBizTotalsByPeriod
AS
	WITH cash_codes AS
	(
		SELECT CategoryCode, CashCode, CashTypeCode, CashPolarityCode
		FROM App.vwTaxBizCashCodes
	),
	invoiced_Projects AS
	(
		...
		FROM            Invoice.tbProject INNER JOIN
								 cash_codes CashCodes  ON Invoice.tbProject.CashCode = CashCodes.CashCode INNER JOIN
	...
	(
		...
		FROM         Invoice.tbItem INNER JOIN
								  cash_codes CashCodes ON Invoice.tbItem.CashCode = CashCodes.CashCode INNER JOIN
	...
	SELECT App.tbYearPeriod.StartOn, netprofit_consolidated.NetProfit, 
							CAST(netprofit_consolidated.NetProfit * App.tbYearPeriod.BusinessTaxRate + App.tbYearPeriod.BusinessTaxAdjustment AS decimal(18, 5)) AS BusinessTax, 
							App.tbYearPeriod.BusinessTaxAdjustment
